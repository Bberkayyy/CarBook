...
public class AdminServiceController : Controller
{
    ...
    public async Task<IActionResult> CreateService(CreateServiceDTO createServiceDTO)
    {
        ...
        var responseMessage = await client.PostAsync("https://localhost:7041/api/Services", content);
        if (responseMessage.IsSuccessStatusCode)
        {
    ...
    {
        var client = _httpClientFactory.CreateClient();
        var responseMessage = await client.GetAsync($"https://localhost:7041/api/Services/{id}");
        if (responseMessage.IsSuccessStatusCode)
        {
            var jsonData = await responseMessage.Content.ReadAsStringAsync();
    ...
    public async Task<IActionResult> UpdateService(UpdateServiceDTO updateServiceDTO)
    {
        ...
        var responseMessage = await client.PutAsync("https://localhost:7041/api/Services", stringContent);
        if (responseMessage.IsSuccessStatusCode)
        {
            return RedirectToAction("Index", "AdminService", new { area = "Admin" });
        }
    ...
    {
        var client = _httpClientFactory.CreateClient();
        var responseMessage = await client.DeleteAsync("https://localhost:7041/api/Services?id=" + id);
        if (responseMessage.IsSuccessStatusCode)
        {
            return RedirectToAction("Index", "AdminService", new { area = "Admin" });
        }
